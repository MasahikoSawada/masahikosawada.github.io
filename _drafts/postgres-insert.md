...
SPI(Server Programming Interface)は、C言語で書かれたユーザ定義関数からSQLを実行する機能です。PostgreSQLの内部処理でも多く利用されており、`REFRESH MATERIALIZED VIEW CONCURRENTLY`でマテリアライズド・ビューとテーブルの差分を取得する所や、PL/pgSQLの内部処理でも使われています。
...
一方で、Executorやその上レイヤーで考慮するべきことはできていません。例えば、このコードではテーブルの存在確認、権限チェック、トリガーの実行、テーブルがパーティションテーブルだったときの対応、などはできていません。
...
先程よりは簡単に書けました。しかし、Executorから実行した時とは異なり、インデックスへのINSERTもやってくれませんし、テーブルがHeap Access Methodであるときにだけこのコードは動きます。

Ok: buffer access, WAL,
missing: index
